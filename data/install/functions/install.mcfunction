...
tellraw @a [{"text":"【更新公告】","color":"gold","bold":true,"italic":false,"underlined":false,"strikethrough":false,"obfuscated":false,"clickEvent":{"action":"run_command","value":"/function install:update_notice"}}]
tellraw @a [{"text":"-------------------------------------------"}]